...
from src.models.xresnet1d import (
    xresnet1d18,
    xresnet1d34,
    xresnet1d50,
    xresnet1d101,
...
from src.models.basic_conv1d import (
    fcn,
    fcn_wang,
    schirrmeister,
    sen,
...
from src.models.base_model import ClassificationModel
import torch
...
from src.utils.utils import evaluate_experiment
...
class metric_func(Callback):
    ...
    def after_batch(self):
        ...
        if self.one_hot_encode_target is True:
            y_true_flat = (
                np.arange(self.pred.size()[-1]) == y_true_flat[:, None]
            ).astype(float)
    ...
    def after_epoch(self):
        ...
        if self.metric_component is not None:
            self.learn.recorder.log.append(self.metric_complete[self.metric_component])
        else:
            self.learn.recorder.log.append(self.metric_complete)
...
class fastai_model(ClassificationModel):
    ...
    def _get_learner(self, X_train, y_train, X_val, y_val, num_classes=None):
        ...
        if self.name.startswith("fastai_lstm") or self.name.startswith("fastai_gru"):
            callbacks.append(GradientClip(max_norm=0.25))
